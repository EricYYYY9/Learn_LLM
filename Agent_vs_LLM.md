...
这里用 `×` 而不是 `+`：LLM 是底层推理能力，规划/记忆/工具是应用层模块，三者必须"乘上" LLM 才能形成 Agent——LLM 不是与它们并列的"第四个模块"，而是它们共同**调用**的推理底座。详见[第三章](#三agent-四模块描述是否正确)与[第五章](#五规划模块和-llm-的关系包含还是调用)。
...
### 6. 面试加分点：Agent 的四个职责层

注意是"**职责层**"而非"独立的并列模块"——LLM 是底层推理内核，其余三者是应用层调用方。面试时建议这么讲：

> "Agent 由四个**职责层**组成：
>
> 1. **LLM 内核**：作为 reasoning engine 提供理解、推理、生成能力，是无状态的；
> 2. **规划层**：基于 LLM 实现 ReAct / Plan-and-Execute 等控制循环，**驱动 LLM 进行任务拆解**，并把 LLM 的输出落地为可执行的步骤序列、错误恢复策略；
...
```
Thought: 知道明天很可能下雨，找到了 evt_001 是跑步事件，根据用户需求，应该删除跑步事件。
